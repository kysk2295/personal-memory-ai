...
describe('buildInitialAppShellEvidenceLayout', () => {
  ...
  test('renders a benchmark-like second-brain graph workspace instead of a dashboard shell', () => {
    ...
    expect(html).toContain('data-benchmark-node-count="225"');
    expect(html).toContain('data-benchmark-edge-count="1010"');
    expect(html).toContain('data-ambient-node-count="225"');
    expect(html).toContain('data-ambient-edge-count="1010"');
    expect(html).toContain('data-graph-density="benchmark-dense"');
    expect(html).toContain('<strong>225</strong> 노드');
    expect(html).toContain('<strong>1010</strong> 엣지');
